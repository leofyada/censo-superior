...
funcao_carregamento <- function() {
  tryCatch({
    ...
    caminho_micro_cur <- here("data", "bronze", "unzip", "microdados_censo_da_educacao_superior_2024", "dados", "MICRODADOS_CADASTRO_CURSOS_2024.CSV")
    caminho_micro_ies <- here("data", "bronze", "unzip", "microdados_censo_da_educacao_superior_2024", "dados", "MICRODADOS_ED_SUP_IES_2024.CSV")
    ...
    cat("Bucket existe!\n")
    # Carrega variável ambiente
    readRenviron(".Renviron")
    # Converte para parquet e carrega para S3
    arrow::write_dataset(dataset = micro_cur, path = "s3://profissaodocente-inepdata-s3/inep/bronze/microsuperior2024_cur", format = "parquet", partitioning = c("TP_MODALIDADE_ENSINO"))
    arrow::write_dataset(dataset = micro_ies, path = "s3://profissaodocente-inepdata-s3/inep/bronze/microsuperior2024_ies", format = "parquet")
    cat("Dados carregados para S3 com sucesso!\n")
    # Exclui pasta zip 
    file.remove(here("data", "bronze", "censosuperior_24.zip"))
